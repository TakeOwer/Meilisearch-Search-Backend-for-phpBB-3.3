...
$lang = array_merge($lang, array(
	...
	'MEILISEARCH_BANNER_VERSION_TITLE'	=> 'Meilisearch Search Backend version %s',
	// Synonyms
	'ACP_MEILISEARCH_SYNONYMS_EXPLAIN'	=> 'Words that should find each other. Meilisearch applies these at query time, so changes take effect on the next search without reindexing.',
	'MEILISEARCH_SYN_LIST'				=> 'Synonym groups',
	'MEILISEARCH_SYN_HOWTO_TITLE'		=> 'How to write the list',
	'MEILISEARCH_SYN_HOWTO'				=> 'One group per line, terms separated by commas. Every term in a group finds every other term, in both directions. A line starting with <samp>#</samp> is a comment. Groups of a single term are ignored, since a word is not its own synonym.<br /><br />Example:<br /><samp>subtitles, subs, sub<br />1080p, fullhd, full hd<br /># product codes<br />r7, r-7</samp><br /><br />Terms are matched case-insensitively, so there is no need to list capitalised variants. Synonyms are a query-time setting: they do not change what is stored, and no reindex is needed.',
	'MEILISEARCH_SYN_FIELD'				=> 'Groups',
	'MEILISEARCH_SYN_FIELD_EXPLAIN'		=> 'Up to 500 groups of up to 20 terms each. Terms longer than 100 characters are ignored.',
	'MEILISEARCH_SYN_COUNT'				=> 'Currently in use',
	'MEILISEARCH_SYN_COUNT_VALUE'		=> '%1$d groups, %2$d terms',
	'MEILISEARCH_SYN_PREVIEW'			=> 'What Meilisearch receives',
	'MEILISEARCH_SYN_PREVIEW_EXPLAIN'	=> 'Meilisearch stores synonyms one way round, so each term is listed with the terms it will also match. This table is what is actually sent: if a pairing is missing here, it will not work on the board.',
	'MEILISEARCH_SYN_TERM'				=> 'Searching for',
	'MEILISEARCH_SYN_MATCHES'			=> 'also finds',
	'MEILISEARCH_SYN_SAVE'				=> 'Save and apply',
	'MEILISEARCH_SYN_SAVED'				=> 'Saved and applied to the index: %d groups are now in use.',
	'MEILISEARCH_SYN_NOT_APPLIED'		=> 'The list was saved but could not be applied to the index: %s',
	'MEILISEARCH_SYN_NEEDS_TWO_TERMS'	=> 'Line %d was ignored: a group needs at least two different terms.',
	'MEILISEARCH_SYN_TOO_MANY_TERMS'	=> 'Line %d has more than 20 terms; the extra ones were dropped.',
	'MEILISEARCH_SYN_TOO_MANY_GROUPS'	=> 'Stopped at line %d: the list may hold at most 500 groups.',

	'MEILISEARCH_ATTACH_NOTE'			=> 'Attachment file names and captions are indexed from version 1.11.0. Existing documents do not have them until the board is reindexed.',
	// Highlighting
	'MEILISEARCH_HIGHLIGHT'				=> 'Highlight fuzzy matches',
	'MEILISEARCH_HIGHLIGHT_EXPLAIN'		=> 'phpBB marks the words the user typed. Meilisearch also returns posts that matched through a typo, a synonym or a different inflection, and those stay unmarked &mdash; so a result appears with nothing highlighted and no visible reason why. With this on, the extension asks Meilisearch which words it actually matched on the page being shown and adds them to phpBB&rsquo;s own highlighting. <strong>Cost:</strong> one extra request per results page, limited to the posts on that page. On a Meilisearch instance reached over the network that is a round trip the visitor waits for, so it is off by default. Check the round-trip time on the diagnostics page before enabling it.',
	// Starter synonym lists
	'MEILISEARCH_SYN_STARTER'			=> 'Starter lists',
	'MEILISEARCH_SYN_STARTER_EXPLAIN'	=> 'Two ready-made lists ship with the extension, one Italian and one English. They cover language names, video and audio formats, content types, common computing terms and the phrases that recur on any board. Adding puts the groups you do not already have into the box above; replacing discards what is there.',
	'MEILISEARCH_SYN_STARTER_NOTE'		=> 'Loading a list only fills the box &mdash; <strong>nothing is saved until you press Save and apply</strong>, so you can review and edit first. The starter files live inside the extension and are replaced on every update, which is deliberate: corrections arrive with the code. Your own list is kept in the database and survives updates untouched.',
	'MEILISEARCH_SYN_REPLACE_CONFIRM'	=> 'This discards everything currently in the box. Continue?',
	'MEILISEARCH_SYN_STARTER_LOADED'	=> 'List loaded: the box now holds %d groups. Press Save and apply to keep them.',
	'MEILISEARCH_SYN_STARTER_MISSING'	=> 'The starter file for &ldquo;%s&rdquo; could not be read. Check that the data folder was uploaded with the extension.',
	'MEILISEARCH_SYN_ADD_X'				=> 'Add the %s list',
	'MEILISEARCH_SYN_REPLACE_X'			=> 'Replace with the %s list',
	'MEILISEARCH_SYN_NO_STARTERS'		=> 'No starter list was found. Check that the data folder was uploaded with the extension.',
	'MEILISEARCH_SYN_ADD_OWN'			=> 'To add a language, drop a file named <samp>synonyms_xx.txt</samp> into the extension&rsquo;s <samp>data</samp> folder, where <samp>xx</samp> is any short code. Put <samp># name: Fran&ccedil;ais</samp> on the first line to give it a readable label. The buttons above are built from what is in that folder, so no code has to be edited. Load it, then press Save and apply: from that moment the list lives in the database and the file has done its job.',
	'MEILISEARCH_SYN_FILTER'			=> 'Filter',
	'MEILISEARCH_SYN_FILTER_PLACEHOLDER'=> 'type a term to find it',
	'MEILISEARCH_SYN_FILTER_NONE'		=> 'No term matches the filter.',
	'MEILISEARCH_SYN_TERMS_LABEL'		=> 'terms shown',
	'MEILISEARCH_HILIT_STYLE'			=> 'Make highlighting easier to see',
	'MEILISEARCH_HILIT_STYLE_EXPLAIN'	=> 'phpBB marks matched words with a muted pink that is hard to pick out, and styles derived from prosilver often soften it further until it is effectively invisible. This replaces it with a pale yellow on the search pages, the colour readers already associate with &ldquo;this is what you searched for&rdquo;. It is a stylesheet rule only: no extra request, no effect on anything but the appearance. Switch it off if your style already highlights the way you want.',
));
