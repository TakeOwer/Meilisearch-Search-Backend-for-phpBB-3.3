...
class indexer
{
	 ...
	 */
	public function __construct(client $client, \phpbb\config\config $config, \phpbb\db\driver\driver_interface $db, \phpbb\textformatter\utils_interface $text_formatter_utils, \phpbb\auth\auth $auth, \phpbb\config\db_text $config_text, $table_prefix)
	{
		$this->client               = $client;
		$this->config               = $config;
		$this->db                   = $db;
	...
	/**
	 * The administrator's synonym list, as written in the ACP.
	 *
	 * Held in config_text rather than config because it is free text with no
	 * useful length limit.
	 *
	 * @return string
	 */
	...
	{
		return (string) $this->config_text->get('meilisearch_synonyms');
	}
	 ...
	 * @return void
	 */
	public function set_synonyms_raw($raw)
	{
		$this->config_text->set('meilisearch_synonyms', (string) $raw);
	}
	...
	public function build_documents(array $post_ids)
	{
		...
		$attachments     = $this->fetch_attachment_names($post_ids);
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$documents[(int) $row['post_id']] = array(
				...
				'attach_names'    => isset($attachments[(int) $row['post_id']]) ? $attachments[(int) $row['post_id']] : '',
			);
		}
	...
	/**
	 * File names and captions of the attachments on a set of posts.
	 *
	 * Neither phpBB's native backend nor this one used to index these, which on
	 * a board where files are shared is probably the most common search that
	 * silently returned nothing. Orphaned rows are skipped: they belong to
	 * uploads that were never attached to a post.
	 *
	 * @param array $post_ids
	 * @return array post_id => space separated names
	 */
	protected function fetch_attachment_names(array $post_ids)
	{
		...
		$sql = 'SELECT post_msg_id, real_filename, attach_comment
			FROM ' . ATTACHMENTS_TABLE . '
			WHERE ' . $this->db->sql_in_set('post_msg_id', array_map('intval', $post_ids)) . '
				AND in_message = 0
				AND is_orphan = 0';
		$result = $this->db->sql_query($sql);
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			$id = (int) $row['post_msg_id'];

			$parts = array((string) $row['real_filename']);

			if (trim((string) $row['attach_comment']) !== '')
			{
				$parts[] = (string) $row['attach_comment'];
			}

			$text = $this->clean_text(implode(' ', $parts), false);

			$names[$id] = isset($names[$id]) ? trim($names[$id] . ' ' . $text) : $text;
		}
