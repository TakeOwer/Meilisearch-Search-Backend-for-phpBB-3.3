...
class meilisearch_backend extends \phpbb\search\base
{
	...
	{
		// The payload is assembled by meili\query_builder, which is a pure
		// function of its arguments and therefore covered by tests. Keeping it
		// out of this class is what makes the filter string verifiable without
		// a phpBB installation.
		$builder = new \salvocortesiano\meilisearch\meili\query_builder();

		$payload = $builder->build($this->search_query, $fields, array(
			'ex_fid_ary'    => $ex_fid_ary,
			'topic_id'      => $topic_id,
			'author_ary'    => $author_ary,
			'author_name'   => (bool) $author_name,
			'sort_days'     => $sort_days,
			'approved_only' => $this->user_sees_approved_only(),
			'limit'         => (int) $this->config['meilisearch_max_results'],
	...
	/**
	 * Does phpBB's visibility rule reduce to "approved" for this user?
	 *
	 * True only when the user holds m_approve in no forum at all. Anyone who
	 * moderates somewhere can legitimately see unapproved and soft-deleted
	 * posts in those forums, and that is not expressible as a single filter, so
	 * for them nothing is pushed down and SQL decides alone - exactly as before.
	 *
	 * @return bool
	 */
	protected function user_sees_approved_only()
	{
		return !$this->auth->acl_getf_global('m_approve');
	}
	...
	/** Relevance ordering modes, see meilisearch_relevance_mode */
	const RELEVANCE_NEVER    = 0;
	const RELEVANCE_IF_UNSET = 1;
	const RELEVANCE_DEFAULT  = 2;

	/**
	 * Should results be ordered by Meilisearch relevance instead of the SQL sort?
	 *
	 * The subtlety this method exists for: phpBB's advanced search form ships
	 * with "Sort results by" pre-selected on post time (sk=t, sd=d). Submitting
	 * that form therefore always sends a sort order, even when the user never
	 * touched the control. Treating any sk as an explicit choice - which is what
	 * mode RELEVANCE_IF_UNSET does - means relevance effectively only ever
	 * applies to the header quick-search box, and a query for two words returns
	 * newest-first rather than best-match-first. That looks like broken matching
	 * and is the single most common complaint about this backend.
	 *
	 * RELEVANCE_DEFAULT therefore treats the form's own pre-selected values as
	 * "no choice made", and only steps aside when the user actually picks
	 * something else.
	 *
	 * @return bool
	 */
	...
	{
		$mode = (int) $this->config['meilisearch_relevance_mode'];

		if ($mode === self::RELEVANCE_NEVER)
		{
			return false;
		}
		...
		if (!$request->is_set('sk'))
		{
			return true;
		}

		if ($mode === self::RELEVANCE_IF_UNSET)
		{
			return false;
		}

		// RELEVANCE_DEFAULT: sk=t with sd=d is exactly what search_body.html
		// pre-selects, so it carries no information about user intent.
	...
	public function acp()
	{
		...
		<dl>
			<dt><label for="meilisearch_relevance_mode">' . $this->lang('MEILISEARCH_RELEVANCE') . $this->lang('COLON') . '</label><br /><span>' . $this->lang('MEILISEARCH_RELEVANCE_EXPLAIN') . '</span></dt>
			<dd>' . $this->relevance_select() . '</dd>
		</dl>
		<dl>
		...
		<dl>
			<dt><label for="meilisearch_highlight">' . $this->lang('MEILISEARCH_HIGHLIGHT') . $this->lang('COLON') . '</label><br /><span>' . $this->lang('MEILISEARCH_HIGHLIGHT_EXPLAIN') . '</span></dt>
			<dd>' . $this->radio('meilisearch_highlight') . '</dd>
		</dl>
		<dl>
			<dt><label for="meilisearch_hilit_style">' . $this->lang('MEILISEARCH_HILIT_STYLE') . $this->lang('COLON') . '</label><br /><span>' . $this->lang('MEILISEARCH_HILIT_STYLE_EXPLAIN') . '</span></dt>
			<dd>' . $this->radio('meilisearch_hilit_style') . '</dd>
		</dl>
		';
		...
		return array(
			...
			'config' => array(
				...
				'meilisearch_relevance_mode' => 'integer:0:2',
				'meilisearch_queue_enable' => 'bool',
				'meilisearch_banner_enable' => 'bool',
				'meilisearch_highlight'     => 'bool',
				'meilisearch_hilit_style'   => 'bool',
			),
		);
	}
	...
	/**
	 * Render the relevance mode selector.
	 *
	 * @return string
	 */
	protected function relevance_select()
	{
		$current = (int) $this->config['meilisearch_relevance_mode'];

		$options = array(
			self::RELEVANCE_DEFAULT  => 'MEILISEARCH_RELEVANCE_DEFAULT',
			self::RELEVANCE_IF_UNSET => 'MEILISEARCH_RELEVANCE_IF_UNSET',
			self::RELEVANCE_NEVER    => 'MEILISEARCH_RELEVANCE_NEVER',
		);

		$html = '<select id="meilisearch_relevance_mode" name="config[meilisearch_relevance_mode]">';
